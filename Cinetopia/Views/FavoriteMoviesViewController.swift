...
extension FavoriteMoviesViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setupView(MovieManager.shared.favoritesMovies[indexPath.item])
        cell.delegate = self
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if kind == UICollectionView.elementKindSectionHeader {
            guard let headeView = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "FavoriteCollectionReusableView", for: indexPath) as? FavoriteCollectionReusableView else {
                fatalError("error to create collectionview header")
            }
            headeView.setupTitle("Meus filmes favoritos")
            return headeView
        }
...
extension FavoriteMoviesViewController: FavoriteMovieCollectionViewCellDelegate {
    func didSelectFavoriteButton(_ sender: UIButton) {
        ...
        let selectedMovie = MovieManager.shared.favoritesMovies[indexPath.row]
